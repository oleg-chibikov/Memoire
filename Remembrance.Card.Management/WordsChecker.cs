...
{
    [UsedImplicitly]
    internal sealed class WordsChecker : IWordsChecker
    {
        ...
        public TranslationInfo CheckWord(string text, string sourceLanguage, string targetLanguage, bool allowExisting, int id)
        {
            logger.Debug($"Checking translation {text} ({sourceLanguage} - {targetLanguage})...");
            ...
            logger.Debug($"Translation check for {text} ({sourceLanguage} - {targetLanguage}) succeeded");
