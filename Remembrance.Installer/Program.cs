...
namespace Remembrance.Installer
{
    internal static class Program
    {
        private static void Main()
        {
            ...
            new InstallBuilder(nameof(Remembrance), nameof(Scar), buildDir, upgradeCode).WithIcon(productIcon).WithShortcut(fileName).WithAutostart(fileName).OpenFolderAfterInstallation().LaunchAfterInstallation(fileName).WithProcessTermination(fileName).Build();
        }
    }
}